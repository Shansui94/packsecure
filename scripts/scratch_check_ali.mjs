...
async function run() {
    const { data } = await supabase.from('delivery_rates').select('location_name, origin, created_at, base_rate, max_places, extra_rate_per_place').ilike('location_name', '%ali%');
    console.log(data);
}
run();
