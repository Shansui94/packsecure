...
async function run() {
    const { data, error } = await supabase.rpc('get_function_def_mock'); // let's just query PG structure using an edge function!
    // Since we don't have RPC, let's just test by doing an insert and looking at the channel events VERY carefully.
}
run();
