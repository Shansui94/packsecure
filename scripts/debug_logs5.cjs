...
async function test() {
  const triggerItemResponse = await supabase.from('stock_ledger_v2').select('*').eq('sku', 'BW-SL-CLR-100Mx100CMx1ROLL-RED').order('timestamp', {ascending: false}).limit(10);
  
  fs.writeFileSync('scripts/debug_out5.json', JSON.stringify({ triggerItemResponse }, null, 2));
}
test();
