-- =========================================================================
-- ROBUST INVENTORY TRIGGERS V2 (JSON sourceLocation aware)
-- =========================================================================
...
-- 3. Create the improved JSON-aware function
    ...
    THEN
        IF NEW.items IS NOT NULL AND jsonb_typeof(NEW.items) = 'array' THEN
            FOR item IN SELECT * FROM jsonb_array_elements(NEW.items) LOOP
                ...
                -- 👉 SMART LOCATION DETECTION: 优先读取 JSON 中的 sourceLocation
                item_loc := NULLIF(TRIM(item.value->>'sourceLocation'), '');
                IF item_loc IS NULL THEN
                    item_loc := NEW.factory_id; -- 使用顶部默认地址
                END IF;
            ...
            FOR item IN SELECT * FROM jsonb_array_elements(OLD.items) LOOP
                ...
                IF item_sku IS NOT NULL AND item_qty IS NOT NULL AND item_qty > 0 THEN
                    INSERT INTO public.stock_ledger_v2 (
                        ...
                        item_qty, 
                        item_loc, 
                        'Auto-refund: Order Cancelled/Deleted', 
                        OLD.order_number
                    );
                END IF;
            END LOOP;
