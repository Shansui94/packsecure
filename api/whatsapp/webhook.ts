...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const rawFrom = msg.from; // e.g., '60123456789'
      ...
      // Step 1: Match sender against users_public
      // Check both raw, with 0 prefix, or without country code
      ...
      if (!employee) {
        ...
        // Check if the user replied with a numeric employee_id / PIN (e.g. "3190", "013")
        const pinMatch = textContent.match(/\b\d{3,6}\b/);
        if (pinMatch) {
          const candidatePin = pinMatch[0];
          // Search for user by employee_id or zero-padded variants
        ...
        // Prompt user to bind PIN/employee_id
      ...
      if (msgType === 'image') {
        ...
        // Driver sending delivery photo / POD
        if (empRole === 'Driver') {
        ...
        // Operator or workshop staff sending scale / scrap photo
        try {
...
Output valid JSON only: { "is_scale": boolean, "weight_kg": number or null, "description": string }`;
      ...
      // ------------------------------------------
      // CASE 2: EMPLOYEE SENDS A TEXT COMMAND
      // ------------------------------------------
