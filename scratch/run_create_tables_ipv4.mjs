...
dns.resolve4('db.kdahubyhwndgyloaljak.supabase.co', async (err, addresses) => {
    if (err || !addresses || addresses.length === 0) {
        console.error('DNS resolve4 failed:', err);
        return;
    }
    const ip = addresses[0];
    console.log("Resolved IPv4 IP:", ip);

    const client = new pg.Client({ 
        host: ip,
        port: 5432,
        user: 'postgres',
        password: 'packsecure2024',
        database: 'postgres',
        ssl: { rejectUnauthorized: false, servername: 'db.kdahubyhwndgyloaljak.supabase.co' }
    });

    try {
        await client.connect();
        console.log("✅ Connected successfully via direct IPv4 IP!");
