...
const AuditReport: React.FC<Props> = ({ user }) => {
    ...
    const filteredSessions = useMemo(() => {
        ...
        return sessions.map(session => {
            const sessionMatch = !search || 
                (session.ref_doc || '').toLowerCase().includes(lowerSearch) ||
                (session.location || '').toLowerCase().includes(lowerSearch) ||
                (session.auditor || '').toLowerCase().includes(lowerSearch);

            let filteredItems = session.items;
            
            if (search && !sessionMatch) {
                filteredItems = session.items.filter(i => 
                    (i.sku || '').toLowerCase().includes(lowerSearch) || 
                    (skuNames[i.sku] || '').toLowerCase().includes(lowerSearch)
                );
            }
            
            return { ...session, items: filteredItems, sessionMatch };
        })
        .filter(session => session.items.length > 0)
        .map(session => {
            if (!search) return session;
            ...
            const net_variance = session.items.reduce((s, i) => s + i.change_qty, 0);
            const var_percent = total_sys > 0 ? ((net_variance / total_sys) * 100).toFixed(1) : '0.0';

            return {
                ...session,
                total_adjustments: session.items.reduce((s, i) => s + Math.abs(i.change_qty), 0),
                net_variance,
                variance_percent: parseFloat(var_percent),
                positive_count: session.items.filter(i => i.change_qty > 0).length,
                negative_count: session.items.filter(i => i.change_qty < 0).length,
                zero_variance_count: session.items.filter(i => i.change_qty === 0).length,
            };
        });
    }, [sessions, search, skuNames]);
    ...
    const totalAbsVariance = filteredSessions.reduce((s, x) => s + x.total_adjustments, 0);

    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#09090b] text-slate-900 dark:text-white p-4 md:p-8 font-sans selection:bg-blue-500/30">
            <div className="max-w-6xl mx-auto">

                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white mb-2 flex items-center gap-3">
                        <ClipboardCheck className="text-blue-600 dark:text-blue-400" size={36} />
                        {t('盘点分析与核销')}
                    </h1>
                    <p className="text-slate-500 dark:text-gray-400 font-medium">{t('历史库存盘点调整、差异分析与管理审核日志')}</p>
                </div>

                {/* Premium Summary Cards */}
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    {[
                        { label: t('总盘点单数'), value: filteredSessions.length, icon: ClipboardCheck, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20 shadow-sm' },
                        { label: t('已核对商品次'), value: filteredSessions.reduce((s, x) => s + x.items.length, 0), icon: PackageSearch, color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20 shadow-sm' },
                        { label: t('总净差异'), value: `${totalNetVariance > 0 ? '+' : ''}${totalNetVariance}`, icon: Activity, color: totalNetVariance >= 0 ? 'text-green-500' : 'text-red-500', bg: totalNetVariance >= 0 ? 'bg-green-500/10' : 'bg-red-500/10', border: totalNetVariance >= 0 ? 'border-green-500/20 shadow-sm' : 'border-red-500/20 shadow-sm' },
                        { label: t('总绝对误差'), value: totalAbsVariance, icon: AlertTriangle, color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/20 shadow-sm' },
                        { label: t('待审核单数'), value: filteredSessions.filter(s => !s.reviewed_by).length, icon: ShieldCheck, color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20 shadow-sm' },
                    ].map(card => (
                        <div key={card.label} className={`${card.bg} ${card.border} backdrop-blur-xl border rounded-3xl p-6 flex flex-col gap-3 group relative overflow-hidden transition-all hover:scale-[1.02]`}>
                            <div className="absolute top-0 right-0 w-32 h-32 hidden -translate-y-1/2 translate-x-1/2 pointer-events-none group-hover:bg-white/10 transition-colors" />
                            <card.icon size={20} className={card.color} />
                            <div>
                                <div className={`text-4xl font-black font-mono tracking-tight ${card.color}`}>{card.value}</div>
                                <div className="text-xs text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mt-1">{card.label}</div>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {/* Filters */}
                <div className="bg-white dark:bg-[#121218] border border-slate-200 dark:border-slate-200 dark:border-white/10 shadow-sm rounded-3xl p-5 mb-8 flex flex-wrap gap-4 items-end relative z-20">
                    <div className="flex-1 min-w-48">
                        <label className="block text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mb-1.5 ml-1">{t('搜索数据库')}</label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all font-medium"
                            />
                        </div>
                    </div>
                    <div className="min-w-40">
                        <label className="block text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mb-1.5 ml-1">{t('库位筛选')}</label>
                        <select
                            value={locFilter}
                            onChange={e => setLocFilter(e.target.value)}
                            className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500/50 transition-all appearance-none cursor-pointer"
                        >
                    ...
                    <div className="min-w-36">
                        <label className="block text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mb-1.5 ml-1">{t('起始日期')}</label>
                        <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                            className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500/50 transition-all [color-scheme:dark]" />
                    </div>
                    <div className="min-w-36">
                        <label className="block text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest mb-1.5 ml-1">{t('截止日期')}</label>
                        <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
                            className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-blue-500/50 transition-all [color-scheme:dark]" />
                    </div>
                    <button onClick={fetchAudits} className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white border border-blue-500/50 shadow-lg shadow-blue-500/20 rounded-2xl text-sm font-bold text-white transition-all active:scale-95">
                ...
                {chartData.length > 0 && !loading && (
                    <div className="bg-[#09090b]/80 backdrop-blur-xl border border-slate-100 dark:border-white/5 rounded-3xl p-6 mb-8 relative overflow-hidden group">
                ...
                {/* Sessions List */}
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 bg-[#09090b]/50 border border-slate-100 dark:border-white/5 rounded-3xl backdrop-blur-sm">
                    ...
                    <div className="text-center py-32 bg-[#09090b]/50 border border-slate-100 dark:border-white/5 rounded-3xl backdrop-blur-sm">
                        <ClipboardCheck size={48} className="mx-auto mb-4 opacity-20 text-white" />
                        <div className="text-lg font-bold text-slate-500 dark:text-gray-400 mb-1">No audit records found</div>
                        ...
                        {filteredSessions.map(session => {
                            ...
                            return (
                                <div key={session.ref_doc} className={`bg-[#0d0d12]/80 backdrop-blur-xl border rounded-3xl overflow-hidden transition-all duration-300 ${
                                    isOpen 
                                        ? 'border-blue-500/50 shadow-[0_0_30px_-5px_rgba(168,85,247,0.15)] ring-1 ring-purple-500/20' 
                                        : isReviewed 
                                            ...
                                            : isMajorLoss 
                                                ...
                                                : 'border-slate-200 dark:border-white/10 hover:border-amber-500/30 hover:bg-white dark:bg-[#12121a] shadow-xl shadow-black/50'
                                }`}>
                                    ...
                                    >
                                        ...
                                        {/* Icon */}
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${isReviewed ? 'bg-green-500/10 shadow-green-500/20' : isMajorLoss ? 'bg-red-500/10 shadow-red-500/20' : 'bg-purple-500/10 shadow-blue-500/20'}`}>
                                            {isReviewed
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col gap-2">
                                            ...
                                            <div className="flex flex-wrap items-center gap-4 text-[11px] sm:text-xs text-slate-500 dark:text-gray-400 font-bold tracking-wide">
                                        ...
                                        {/* Dynamic Stats Row */}
                                        <div className="flex items-center justify-between md:justify-end gap-3 lg:gap-5 shrink-0 mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-none border-slate-100 dark:border-white/5">
                                            ...
                                            <div className="hidden sm:block text-center px-4 border-r border-slate-200 dark:border-white/10">
                                            ...
                                            <div className="pl-2">
                                                <div className={`p-2 rounded-full transition-colors ${isOpen ? 'bg-purple-500/20 text-purple-300' : 'bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-gray-400'}`}>
                                    ...
                                    {isOpen && (
                                        <div className="border-t border-slate-100 dark:border-white/5 bg-[#09090b]">
                                            {/* Action Bar */}
                                            <div className="px-6 py-4 flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 bg-gradient-to-r from-purple-500/5 to-transparent">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 text-xs font-bold px-4 py-2 border border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:bg-white/5 rounded-xl text-gray-300 transition-colors shadow-sm"
                                                    >
                                                                ...
                                                                return (
                                                                    <tr key={item.txn_id} className={`transition-colors ${idx % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.01]'} hover:bg-white/[0.03]`}>
                                                                        ...
                                                                        <td className="px-4 py-3 text-right">
                                                                            <span className={`inline-flex min-w-16 justify-center items-center gap-1.5 font-black font-mono text-[11px] px-2.5 py-1 rounded-lg border ${pos ? 'text-green-400 bg-green-500/10 border-green-500/20' : neg ? 'text-red-400 bg-red-500/10 border-red-500/20' : 'text-gray-500 bg-slate-100 dark:bg-white/5 border-transparent'}`}>
