    ...
    return (
        <div className="min-h-screen bg-apple-bg text-apple-textMain font-sans selection:bg-blue-500/30 transition-colors duration-300">
            ...
            {!hideGlobalMobileHeader && (
                <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-apple-bg/90 backdrop-blur-md border-b border-apple-border flex justify-between items-center px-3 z-50 transition-colors duration-300">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <button
                            type="button"
                            onClick={() => window.dispatchEvent(new CustomEvent('packsecure:open-omni-command'))}
                            title="万能输入口 (⌘K)"
                            className="w-8 h-8 rounded-full bg-indigo-950/60 border border-indigo-500/40 flex items-center justify-center text-indigo-300 shadow-md active:scale-95 transition-all mr-1 cursor-pointer"
                        >
                            <Search size={15} />
                        </button>
            ...
            <div className="flex h-screen overflow-hidden pt-16 lg:pt-0">
                ...
                {/* Main Content Area */}
                <main className="flex-1 overflow-y-auto bg-apple-bg relative scroll-smooth selection:bg-purple-500/30 transition-colors duration-300 flex flex-col">
                    {/* 🖥️ Desktop Global Command Bar Header */}
                    <header className="hidden lg:flex items-center justify-between px-6 py-2.5 bg-apple-surface/70 backdrop-blur-md border-b border-apple-border/60 sticky top-0 z-30 transition-colors shrink-0">
                        <div className="flex items-center gap-3 flex-1 max-w-xl">
                            <button
                                type="button"
                                onClick={() => window.dispatchEvent(new CustomEvent('packsecure:open-omni-command'))}
                                className="w-full flex items-center justify-between px-3.5 py-1.5 bg-zinc-900/60 hover:bg-zinc-900/90 border border-zinc-700/70 hover:border-indigo-500/60 rounded-xl text-xs text-zinc-400 shadow-sm transition-all group cursor-pointer"
                            >
                                <div className="flex items-center gap-2">
                                    <Search className="w-3.5 h-3.5 text-zinc-400 group-hover:text-indigo-400 transition-colors" />
                                    <span className="truncate">全局搜索单据、机台、客户或业务指令...</span>
                                </div>
                                <kbd className="flex items-center gap-0.5 font-mono text-[10px] bg-zinc-800 border border-zinc-700 px-1.5 py-0.5 rounded text-zinc-400">
                                    <span>⌘K</span>
                                </kbd>
                            </button>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                type="button"
                                onClick={() => setShowLangModal(true)}
                                className="px-2.5 py-1 rounded-lg bg-zinc-800/60 hover:bg-zinc-800 border border-zinc-700/60 text-xs font-bold text-amber-300 flex items-center gap-1.5 transition cursor-pointer"
                            >
                                <span>{LANGUAGES.find(l => l.code === currentLanguage)?.flag || '🌐'}</span>
                                <span>{LANGUAGES.find(l => l.code === currentLanguage)?.label.split(' ')[0] || '语言'}</span>
                            </button>
                            <button onClick={toggleTheme} className="p-1.5 text-gray-400 hover:text-amber-400 transition-colors cursor-pointer">
